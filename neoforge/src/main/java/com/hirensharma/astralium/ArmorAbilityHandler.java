...
public final class ArmorAbilityHandler {
    ...
    public static void tick(ServerPlayer player) {
        ...
        if (fullSet) {
            if (FULL_SET_ACTIVE.add(RuntimeMappings.uuid(player))) ModCriteriaTriggers.FULL_ARMOUR.trigger(player);
        } else {
            FULL_SET_ACTIVE.remove(RuntimeMappings.uuid(player));
        }
        if (RuntimeMappings.onGround(player) || VoidFloorHandler.isSupported(player)) {
    ...
    public static boolean tryDoubleJump(ServerPlayer player) {
        if (!hasFullSet(player)
            || RuntimeMappings.onGround(player)
            || player.isPassenger()
            || player.isFallFlying()
            || player.isInWater()
            || player.isInLava()
    ...
    public static void clear(ServerPlayer player) {
        LAST_REQUEST_TICKS.remove(RuntimeMappings.uuid(player));
        FULL_SET_ACTIVE.remove(RuntimeMappings.uuid(player));
        player.getPersistentData().remove(DOUBLE_JUMP_USED_TAG);
    }
    ...
    private static boolean isRateLimited(ServerPlayer player) {
        ...
        Long previous = LAST_REQUEST_TICKS.get(RuntimeMappings.uuid(player));
        if (previous != null && now - previous < REQUEST_COOLDOWN_TICKS) return true;
        LAST_REQUEST_TICKS.put(RuntimeMappings.uuid(player), now);
        return false;
    }
}
