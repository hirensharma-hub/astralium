...
public class CycleMiningModePacket {
    ...
    public void handle(Supplier<NetworkEvent.Context> contextSupplier) {
        ...
        context.enqueueWork(() -> {
            ...
            Long previousCycle = LAST_CYCLE_TICKS.get(RuntimeMappings.uuid(player));
            ...
            LAST_CYCLE_TICKS.put(RuntimeMappings.uuid(player), gameTime);
            ...
            if (!tool.hasAnyEnabledMode()) {
                ...
                EnumSet<ExcavationTool> warned = WARNED_DISABLED.computeIfAbsent(
                    RuntimeMappings.uuid(player), ignored -> EnumSet.noneOf(ExcavationTool.class));
                if (warned.add(tool)) {
                    player.displayClientMessage(RuntimeMappings.translatable("message.astralium.excavation_no_modes"), true);
                }
                return;
            }
            MiningMode mode = AstraliumPickaxeItem.cycleMode(stack, tool);
            ModNetworking.sendMiningMode(player, slot, tool, mode);
                    player.displayClientMessage(RuntimeMappings.translatable("message.astralium.excavation_mode", RuntimeMappings.translatable("item.astralium.astralium_" + tool.translationKey()), mode.displayName()), true);
                    RuntimeMappings.level(player).playSound(null, player.blockPosition(), (net.minecraft.sounds.SoundEvent) RuntimeMappings.staticField(SoundEvents.class, "AMETHYST_BLOCK_HIT", "f_144245_"), SoundSource.PLAYERS, 0.45F, 1.45F);
        });
        context.setPacketHandled(true);
    }

    public static void clear(ServerPlayer player) {
        WARNED_DISABLED.remove(RuntimeMappings.uuid(player));
        LAST_CYCLE_TICKS.remove(RuntimeMappings.uuid(player));
    }
