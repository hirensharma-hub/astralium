...
public class AstraliumHoeItem extends HoeItem {
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (current.is(Blocks.FARMLAND) && AstraliumConfig.STARLIT_SOIL_ALLOW_FARMLAND_UPGRADE.get()) {
            target = ModBlocks.ASTRAL_FARMLAND.get().defaultBlockState().setValue(net.minecraft.world.level.block.FarmBlock.MOISTURE, current.getValue(net.minecraft.world.level.block.FarmBlock.MOISTURE));
        ...
        level.playSound(player, position, SoundEvents.AMETHYST_BLOCK_CHIME, SoundSource.BLOCKS, 0.45F, 1.45F);
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag flag) {
        tooltip.add(Component.translatable("tooltip.astralium.hoe.title").withStyle(ChatFormatting.LIGHT_PURPLE));
        tooltip.add(Component.translatable("tooltip.astralium.hoe.description").withStyle(ChatFormatting.GRAY));
        tooltip.add(Component.translatable("tooltip.astralium.hoe.growth").withStyle(ChatFormatting.GRAY));
    }
}
